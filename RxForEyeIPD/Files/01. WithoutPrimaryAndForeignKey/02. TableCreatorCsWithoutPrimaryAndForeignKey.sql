...
select * into TableCreatorFirst from OPENROWSET('Microsoft.ACE.OLEDB.12.0',  'Excel 8.0;HDR=YES;Database=E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Files\01. WithoutPrimaryAndForeignKey\TableCreateWithoutPrimaryAndForeignKey.xlsx', 'select * from [TblUserRole$]') 
...
-- Class Intermace Methods
        ...
        set @TableCreatorString = (select 'echo { >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\CsScript\\'+ @TableName +'.cs"') EXEC xp_cmdshell @TableCreatorString
            ...
            set @TableCreatorString = (select 'echo { >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\CsScript\\'+ @TableName +'.cs"') EXEC xp_cmdshell @TableCreatorString
                ...
                set @TableCreatorString = (select 'echo cmd.Parameters.Add("@'+ (select ColumnName from TableCreator where particulars in ('PrimaryID')) + '", SqlDbType.'+ (select ValueType from TableCreator where particulars in ('PrimaryID')) + ').Value = '+ @PassVarShortName +'.'+ (select ColumnName from TableCreator where particulars in ('PrimaryID')) + ' ?? (object)DBNull.Value; >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\CsScript\\'+ @TableName +'.cs"') EXEC xp_cmdshell @TableCreatorString
            ...
            set @TableCreatorString = (select 'echo { >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\CsScript\\'+ @TableName +'.cs"') EXEC xp_cmdshell @TableCreatorString
				...
				begin
					...
					set @TableCreatorString = (select 'echo cmd.Parameters.Add("@'+ (select ColumnName from TableCreator where particulars in ('Column') and SlNo = @Counter) +'", SqlDbType.'+ (select ValueType from TableCreator where particulars in ('Column') and SlNo = @Counter) +').Value = '+ @PassVarShortName +'.'+ (select ColumnName from TableCreator where particulars in ('Column') and SlNo = @Counter) +' ?? (object)DBNull.Value; >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\CsScript\\'+ @TableName +'.cs"') 
