...
set @TableCreatorString = (select 'echo @inject I'+ (select tbl from TableCreator where Method = 'Yes') +' '+ lower((select tbl from TableCreator where Method = 'Yes')) +' >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
	...
	while ((select (count(*) + 3) Total from TableCreator  where particulars in ('Column')) > @Counter)
		begin
			...
			-- EditForm Each control
				...
				set @TableCreatorString = (select 'echo ^<label^>'+ (select ColumnName from TableCreator where SlNo = @Counter) +'^<^/label^> >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
				
				if exists (select * from TableCreator where SlNo = @Counter and Method = 'Yes')
					begin
						-- Dropdown Section
						
						set @TableCreatorString = (select 'echo ^<SfDropDownList TValue="int" TItem="'+ (select Tbl from TableCreator where SlNo = @Counter) +'Entity" Placeholder="Select '+ (select Txt from TableCreator where SlNo = @Counter) +'" DataSource="@'+ (select Txt from TableCreator where SlNo = @Counter) +'s" @bind-Value="@' + @TableName +'Entity.'+ (select Val from TableCreator where SlNo = @Counter) +'"^> >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
						set @TableCreatorString = (select 'echo ^<DropDownListFieldSettings Value="'+ (select Val from TableCreator where SlNo = @Counter) +'" Text="'+ (select Txt from TableCreator where SlNo = @Counter) +'"^>^<^/DropDownListFieldSettings^> >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
						set @TableCreatorString = (select 'echo ^<^/SfDropDownList^> >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
...
set @TableCreatorString = (select 'echo private List^<'+ (select Tbl from TableCreator where Method = 'Yes') +'Entity^> '+ (select Txt from TableCreator where Method = 'Yes') +'s = new(); // List of Foreign Key Columns >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
...
set @TableCreatorString = (select 'echo  '+ (select Txt from TableCreator where Method = 'Yes') +'s = await '+ (select lower(Tbl) from TableCreator where Method = 'Yes') +'.GetAll'+ (select Tbl from TableCreator where Method = 'Yes') +'(); // add this line to OnInitilizeAsync >> "E:\DotNetCoreProjects\RxForEyeIPD\RxForEyeIPD\Components\Pages\DynamicFileCreator\' + @TableName +'\ComponentScript\SaveAndUpdate\SaveAndUpdate'+ upper(substring(@TableName, 1,1)) + substring(@TableName, 2,50) +'.razor"') EXEC xp_cmdshell @TableCreatorString
